...
/**
 * Generate a short human-readable Chinese label for a run_code script,
 * derived from what the code actually does (comments / web_search query /
 * curled URL) rather than the raw English description argument.
 */
export function describeScript(code, description = "") {
  if (!code || typeof code !== "string") return description || "";
  // 1. 中文注释优先（作者自己写的意图说明）
  for (const line of code.split("\n")) {
    const m = line.match(/^\s*\/\/\s*(.{2,60})$/);
    if (m) {
      const t = m[1].trim();
      if (/[\u4e00-\u9fff]/.test(t)) return t;
    }
  }
  // 2. web_search(query) → 联网搜索（分别匹配单/双引号包裹，容忍内部另一引号）
  const ws1 = code.match(/web_search\(\s*\{\s*query:\s*'([^']{2,200})'/);
  const ws2 = code.match(/web_search\(\s*\{\s*query:\s*"([^"]{2,200})"/);
  const wsQ = (ws1?.[1] || ws2?.[1] || "");
  if (wsQ) return `联网搜索：${wsQ.trim()}`;
  // 3. curl / fetch 抓 URL → 抓取网页（跳过模板字符串 ${...}）
  const curl = code.match(/curl\s+[^"']*?["'](?!\$\{)([^"']{4,120})["']/);
  const fet = code.match(/fetch\(\s*["'](?!\$\{)([^"']{4,120})["']/);
  const url = (curl?.[1] || fet?.[1] || "");
  if (url) {
    const clean = url.replace(/^https?:\/\//, "").slice(0, 60);
    return `抓取网页：${clean}`;
  }
  // 4. 首行可读代码
  const first = code.split("\n").map((s) => s.trim()).filter(Boolean)[0];
  return first ? first.slice(0, 50) : description || "";
}
...
export class ScriptLibraryPlugin extends TypertRemoteService {
  ...
  onToolResult(session, event) {
    ...
    // Human-readable identity: 中文说明，从代码本身提取（注释/搜索词/URL），
    // 避免英文 description 又长又难懂。fallback 到 description。
    const code = (call?.args && typeof call.args === "object" ? call.args.code : "") || "";
    const description = (call?.description || "").trim();
    const label = describeScript(code, description) || description.slice(0, 60) || "执行脚本";
    const base = toKebab((description || label).slice(0, 40));
    const id = `${base || "run"}-${Date.now().toString(36).slice(-4)}`;
    const entry = {
      id,
      type: "待定",
      name: label,
      summary: `脚本内容约 ${content.length} 字符${description ? " · " + description : ""}`,
